...
const Projects = () => {
  ...
  return (
    <div className='bg-white h-[90vh] w-screen flex flex-col items-center justify-evenly'>
      ...
      <div className='grid grid-rows-2 grid-cols-3 w-[65%] -mt-10'>
          {
            projectList.map((item, index) => {
              return(
                <Link href={item.link} target='blank'>
                  <div key={index} className='m-4 hover:scale-105 duration-300 flex flex-col justify-between'>
                    <p className='text-[#2d2e32] text-lg font-bold'>{item.projectName}</p>
                    <p className='text-[#555555] text-base'>{item.projectData}</p>
                    <div className='flex justify-around items-center'>
                      {Array.isArray(item.techStack) && item.techStack.length > 0 && (
                        item.techStack.map((tech, index) => {
                          return(
                            <div key={index} className="flex items-center justify-center h-6 w-[4.5rem] text-sm border-solid rounded-md border-2 border-[#555555] bg-[#555555] text-white">
